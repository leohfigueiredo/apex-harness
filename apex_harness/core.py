...
class ApexAgent:
    ...
    def compact(self) -> str:
        ...
        Strategy (DA-aware):
          ...
          2. Build a 20-line narrative summary of the older history.
          3. Preserve the last 3 tool result messages verbatim so the agent
             doesn't lose the most recent execution context.
          4. Keep the last user message so the current task is not lost.
        """
        ...
        # Step 3 — summarise everything except the tail anchors
        ...
        summary_text = "Resumo do contexto anterior compactado:\n" + "\n".join(summary_lines[:20])

        # Step 4 — assemble the new, compact history
        new_hist = [
            {"role": "system", "content": self.system_prompt},
            {"role": "user", "content": summary_text},
            {"role": "assistant", "content": "Entendido. Contexto anterior resumido. Resultados de tools recentes preservados abaixo."},
        ]
        ...
        return f"Contexto compactado com sucesso! Reduzido para {len(self.history)} blocos essenciais."
    ...
    def _should_provide_tools(self, user_text: str) -> bool:
        """
        Pi-Harness Optimization: Dynamic Tool Pruning.
        If user request is purely conversational, explanatory, or conceptual,
        we omit heavy tool schemas to save 2000-4000 prefill tokens and accelerate TTFT.
        """
        if not self.tools:
            return False
        
        # Always provide tools if command-like or explicitly action-oriented
        text_lower = user_text.lower().strip()
        action_triggers = [
            "pesquise", "search", "busque", "procure", "leia", "read", "veja", "liste",
            "edite", "edit", "crie", "create", "escreva", "write", "execute", "rode", "bash",
            "terminal", "comando", "run", "teste", "test", "corrija", "fix", "rag",
            "http", "www.", ".py", ".md", ".sh", ".json", "/", "diff", "git"
        ]
        
        # If any action keyword is found, keep all tools
        if any(kw in text_lower for kw in action_triggers):
            return True

        # Short conversational greetings / pure conceptual explanations can skip tool overhead
        conversational_starters = ["olá", "ola", "oi", "hello", "hi", "bom dia", "boa tarde", "boa noite", "obrigado", "valeu", "valeu!", "ok", "beleza"]
        if text_lower in conversational_starters or len(text_lower) < 15:
            return False

        # Default to providing tools
        return True
    ...
    def step(
        ...
        # Pi-Harness: Selectively prune tools to accelerate prefill and maximize KV-cache hits
        active_tools = self.tools if self._should_provide_tools(user_input) else None
        ...
        while turns < self.max_turns:
            ...
            for tc in parsed_tool_calls:
                ...
                # Pi-Harness: Restrict oversized raw tool scratchpads to protect KV-cache bandwidth
                max_chars = 12000
                if len(result_str) > max_chars:
                    result_str = result_str[:max_chars] + f"\n... [Truncado de {len(result_str)} para {max_chars} chars pelo Apex Pi-Optimizer]"
