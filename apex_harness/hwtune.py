...
class TuneOptions:
    ...
    #: prio=2 (high) is safe on a single-user workstation and requires no
    #: special capabilities (CAP_SYS_NICE is only needed for prio=3/realtime).
    #: Set prio=0 to restore OS-default scheduling.
    prio: int = 2
    #: scheduling priority for the batch (prefill) thread pool.
    prio_batch: int = 2
    ...
    #:   mmap+mlock  — mmap for fast startup, then mlock to pin against swap (default).
    #:                 Best on 96 GB machines where the whole model fits in RAM.
    #:   mlock       — read weights fully into RAM (no mmap). Use for models on
    #:                 slow/NTFS disks where mmap page eviction causes decode stalls.
    #:   mmap        — mmap only, no pinning. Appropriate on machines with little RAM.
...
def plan_server_command(
    ...
    if opts.speculative:
        ...
        if opts.use_mtp and prof.has_mtp:
            ...
            notes.append(
                ...
                "Measured on this host with a THINKING model (Qwen3.8-27B): the model emitted "
                "<think> prose instead of re-emitting the file, acceptance was 0.00% "
                "(0/96), and decode fell from 3.375 to 1.191 t/s. Rule: keep a speculator "
                "only if acceptance >= ~0.6 AND it beats the no-spec baseline on the same run; "
                "below ~0.4 you are in the Phi-2 regime (0.66x/0.48x). "
                "For thinking models either disable speculation "
                "(TuneOptions(speculative=False)) or disable thinking."
            )
    ...
    argv += ["-lm", opts.load_mode]
    notes.append(
        f"-lm {opts.load_mode}: model loading mode. 'mmap+mlock' is the default — "
        f"weights are mmap'd for fast startup then pinned with mlock to prevent "
        f"swap evictions on this 96 GB machine. Use 'mlock' for models on slow "
        f"(NTFS/spinning) disks. Replaces deprecated --mlock / --mmap / --no-mmap flags."
    )

    # ------------------------------------------------------------ scheduling --
    # prio=2 (high) is safe on a single-user workstation and requires no special
    # capabilities — it maps to SCHED_OTHER with a raised nice value, not realtime.
    # prio=3 (realtime) needs CAP_SYS_NICE and will print an error without it.
    # We also set --prio-batch to keep the prefill threads at the same priority.
    argv += ["--prio", str(opts.prio), "--prio-batch", str(opts.prio_batch)]
    notes.append(
        f"--prio {opts.prio} --prio-batch {opts.prio_batch}: high scheduling priority "
        f"for decode and batch threads. Safe without CAP_SYS_NICE (prio ≤ 2). "
        f"Set TuneOptions(prio=0, prio_batch=0) to restore OS-default scheduling."
    )
